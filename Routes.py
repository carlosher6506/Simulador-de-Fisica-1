...
def draw_trajectory(u, theta, gravity, xPositions=[], yPositions=[], initial_position=(50, 179)):
    ...
    original_max_height = max([u * math.sin(theta) * t - 0.5 * gravity * t * t for t in frange(0, original_t_flight, 0.01)])
    projectile_range = max(xPositions)
    original_projectile_range = max([u * math.cos(theta) * t for t in frange(0, original_t_flight, 0.01)])

    return original_t_flight, original_max_height, original_projectile_range, original_t_flight, original_max_height, original_projectile_range, u
...
while running:
    ...
    if len(x) > 50 and len(y) > 0:
        xCoordinate = int(x.pop(50))
        ...
        for i in range(len(historial_puntos) - 1):
            pygame.draw.line(screen, (255, 255, 255), historial_puntos[i], historial_puntos[i + 1], 2)
